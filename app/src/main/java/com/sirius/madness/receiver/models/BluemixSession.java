...
public class BluemixSession extends IBMDataObject {
    ...
    private static final String IS_GENERAL_SESSION = "isGeneralSession";
    ...
    public void initialize() {
        ...
        object = getObject(SLOT);

        if(object instanceof String) {
            session.setSlot((String) object);
        }else {
            Log.e(CLASS_NAME, "Attempting to retrieve slot failed, as the database returned a non-String data type");
        }
        ...
        object = getObject(IS_GENERAL_SESSION);
        ...
        if(object instanceof Boolean)
        {
            ...
            if(newObject == true) {
                session.setIsGeneralSession("true");
            }else {
                session.setIsGeneralSession("false");
            }
        }else if(object instanceof Integer) {
            Integer number = (Integer) object;
            ...
            if(number == 0) {
                ...
                Log.e(CLASS_NAME, "Attempting to retrieve isGeneralSession failed, as the database returned an Integer that could not be parsed to a Boolean");
            }
        }else {
            Log.e(CLASS_NAME, "Attempting to retrieve isGeneralSession failed, as the database returned a data type that was neither Boolean nor Integer");
        }
    }
