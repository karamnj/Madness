...
public class BluemixSurveyQuestions extends IBMDataObject {
    ...
    public void initialize() {
        ...
        object = getObject(OPTION1);

        if (object instanceof String) {
            surveyQuestions.setOption1((String) object);
        } else {
            Log.e(CLASS_NAME, "Attempting to retrieve Option1 failed, as the database returned a non-string data type");
        }
        object = getObject(OPTION2);

        if (object instanceof String) {
            surveyQuestions.setOption2((String) object);
        } else {
            Log.e(CLASS_NAME, "Attempting to retrieve Option2 failed, as the database returned a non-string data type");
        }
        object = getObject(OPTION3);

        if (object instanceof String) {
            surveyQuestions.setOption3((String) object);
        } else {
            Log.e(CLASS_NAME, "Attempting to retrieve Option3 failed, as the database returned a non-string data type");
        }
    }
